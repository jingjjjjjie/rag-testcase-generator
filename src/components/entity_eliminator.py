...
from src.utils.api_utils import call_api_qwen, get_qwen_embeddings
...
class EntityEliminator:
    ...
    def group_entities_by_name(self, all_entities):
        ...
        It organizes entities into buckets based on their "entity_name" so you can easily access all entities with the same name.
        """
    ...
    def resolve_single_conflict_with_same_name(self, entity_name, entity_group, max_attempts=3):
        ...
        origin_prompt = f"""Here are multiple entity descriptions with the name "{entity_name}":
            ...
            **DOMAIN CONTEXT**: This is an employee FAQ document. Common role entities such as "Worker", "Employee", "Immediate Superior"/"Superior", "Designated Interpreter", "Management" typically refer to the same generic role type across different contexts and should be MERGED.

            Please classify these descriptions:
            1. If the descriptions are IDENTICAL or convey essentially the same meaning (especially for generic role entities), merge them into a single entity and assign it a single consistent name.
            2. If the descriptions convey fundamentally different meanings, generate more specific names and descriptions for each distinct entity. Ensure that the more specific names clearly reflect the different meanings.
            3. You must not skip the processing of any entity.

            **Examples**:
            - MERGE: Multiple "Worker" entities with identical descriptions like "A Worker is an individual employed under a work arrangement..." should be merged into ONE entity.
            - KEEP SEPARATE: "HARVARD UNIVERSITY" and "DUKE UNIVERSITY" are both "UNIVERSITY," but their descriptions refer to different institutions, so they should be treated as separate entities.
        ...
        try:
            ...
            while attempt < max_attempts:
                attempt += 1
                try:
                    response, prompt_tokens, completion_tokens = call_api_qwen(prompt, temperature=self.TEMPERATURE)
    ...
    def resolve_conflicts_with_same_name(self, all_entities):
        ...
        info(f"Grouped into {len(grouped_entities)} unique entity names.")
        to_resolve_len_2 = sum([1 for entity_name, entity_group in grouped_entities.items() if len(entity_group) >= 2])
        info(f"Total {to_resolve_len_2} entity groups with same name to resolve.")
        info(f"Maximum group size: {max(len(group) for group in grouped_entities.values())}")
        ...
        with ThreadPoolExecutor(max_workers=self.NUM_WORKERS) as executor:
            ...
            for future in tqdm(as_completed(future_to_entity_group), total=len(future_to_entity_group), desc="Resolving entity groups...", dynamic_ncols=True):
                ...
                try:
                    ...
                    error(f"Error resolving conflicts: {e}")
    ...
    def resolve_conflicts_in_large_group_with_different_name(self, all_entities, max_chunk_size=10, max_attempts = 2):
        ...
        while True:
            ...
            random.shuffle(entities_to_process) # (打乱顺序，避免顺序bias)
            
            # Process in chunk, max chunk size is set to 10 in this case
            chunks = [entities_to_process[i:i + max_chunk_size] for i in range(0, len(entities_to_process), max_chunk_size)] 
            resolved_entities = []
            for chunk in chunks:
                # use llm to resolve each chunk, the prompt criteria is:
                # 1. If the descriptions convey the same meaning, merge them into a single entity and assign it a single consistent name.
                # 2. If the descriptions convey different meanings, generate more specific names and descriptions for each distinct entity. Ensure that the more specific names clearly reflect the different meanings.
                # 3. You must not skip the processing of any entity.
                resolved_chunk = self.resolve_single_conflict_with_different_name(chunk)
                for resolved_entity in resolved_chunk:
                # Fallback if fail: Map all entities in the cluster to their original names/descriptions
                # return [
                #     {
                #         "entity_ids": [entity["entity_id"]],
                #         "entity_name": entity["entity_name"],
                ...
                #     for entity in entity_group
                # ]
                    # Use the smallest entity_id as the representative
                    smallest_entity_id = min(resolved_entity["entity_ids"]) 
                    for entity_id in resolved_entity["entity_ids"]:
                        entityid2entityid[entity_id] = smallest_entity_id
                    resolved_entities.append({
    ...
    def resolve_conflicts_with_different_name(self, all_entities):
        ...
        # similarity matrix is an n × n matrix, where n is the number of entities.
        ...
        info(f"Computed similarity matrix of shape {similarity_matrix.shape}.")
        ...
        # 如果有100个实体，相似度矩阵是100×100 = 10,000个pair。如果直接让LLM处理所有可能的组合，会：
        #    成本太高（太多LLM调用）
        #    效率太低（大部分pair完全不相关）
        clusters = self.cluster_entities(all_entities, similarity_matrix, threshold)
        info(f"Generated {len(clusters)} clusters with different name for resolution.")
        info(f"Maximum cluster size: {max(len(cluster) for cluster in clusters)}")
        ...
        with ThreadPoolExecutor(max_workers=self.NUM_WORKERS) as executor:
            future_to_cluster = {
                ...
                for cluster in clusters if len(cluster) >= 2 # 分成多少个cluster call 多少次
            }
    ...
    def run(self, inputs=None):
        ...
        # Skip file existence check in direct mode
        if not direct_mode:
            if os.path.exists(self.ENTITY_ELIMINATOR_OUTPUT_PATH):
                info(f"Entity mapping already exists at {os.path.relpath(self.ENTITY_ELIMINATOR_OUTPUT_PATH, PROJECT_ROOT)}.")
                return

        # Load the input data if not provided
        if inputs is None:
        ...
        info(f"Total {len(all_entities)} entities to resolve.")
        ...
        # Save cache only if not in direct mode
        if not direct_mode:
            ...
            info(f"Resolved entities with same name saved to cache/resolved_entities_same_name_{base_name}.json.")
        ...
        if not direct_mode:
            ...
            with open(f"cache/resolved_entities_different_name_{base_name}.json", "w", encoding="utf-8") as f:
                json.dump(resolved_entities, f, indent=2, ensure_ascii=False)
            info(f"Resolved entities with different name saved to cache/resolved_entities_different_name_{base_name}.json.")
        ...
        # Save the results only if not in direct mode
        if not direct_mode:
